...
public func insertMarks(declarations: [SourceDeclaration], limit: NSRange? = nil) -> [SourceDeclaration] {
    ...
    let currentMarks = file.linesContainer.pragmaMarks(filename: path, excludeRanges: declarations.map({
        file.linesContainer.byteRangeToNSRange(start: $0.range.location, length: $0.range.length) ?? NSRange()
    }), limit: limit)
    let newDeclarations: [SourceDeclaration] = declarations.map { declaration in
        var varDeclaration = declaration
        let range = file.linesContainer.byteRangeToNSRange(start: declaration.range.location, length: declaration.range.length)
